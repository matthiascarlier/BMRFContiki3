...
'''
configurations_names = ['BMRF_BROADCAST_1']
scenarios = ['100']
topologies = ['topology_1_childZ1_nodata']
		...
		for mcast_conf in configurations_names:
			...
			for topology in topologies:
				...
				for i in xrange(1, iterations+1):
					print(macProtocol + ":" + topology+ ":"+scenario+ ":"+mcast_conf + ":" + str(i)) 
					...
					with open(path + "raw_results_" + scenario + "-percent_" + mcast_conf + "_iteration-" + str(i) + ".csv", 'rU') as fraw:
						...
						for row in reader:
							if(not row[3]=='n'):
									...
									print((int(row[6])*RX_CONS + int(row[7])*TX_CONS + int(row[8])*LPM_CONS + int(row[9])*CPU_CONS)/(1000*CLOCK_SECOND))
